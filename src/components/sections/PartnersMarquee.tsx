...
const CATEGORIES = [
  ...
  { name: "Live streaming", icon: Sparkles },
];
...
export default function PartnersMarquee() {
  ...
  return (
    <section
      className="border-y border-white/[0.08] bg-[#111111] py-5 sm:py-6"
      aria-label="Studio categories"
    >
      <div className="mx-auto flex max-w-7xl items-center gap-5 px-4 sm:px-6 lg:px-8">
        <span className="hidden shrink-0 font-mono text-[10px] uppercase tracking-[0.18em] text-white/40 sm:block">
          Create in your format
        </span>
        ...
        <div className="min-w-0 flex-1 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)]">
          <div className="marquee-track hover:[animation-play-state:paused]">
            {doubled.map(({ name, icon: Icon }, i) => (
              <Link
                key={`${name}-${i}`}
                href={`/rooms?category=${encodeURIComponent(name.toLowerCase())}`}
              ...
              >
                <Icon
                  className="h-4 w-4 text-white/35 transition-colors group-hover:text-[#c8e6c9]"
                  aria-hidden="true"
                />
        ...
        <motion.div whileHover={{ x: 2 }} className="hidden shrink-0 lg:block">
          <Link
            href="/rooms"
            className="inline-flex min-h-0 items-center gap-1.5 text-xs font-medium text-[#c8e6c9] transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8e6c9]"
          >
            Browse all
