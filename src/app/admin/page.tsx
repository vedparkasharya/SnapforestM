...
function getAdminToken(): string | null {
  try {
    ...
    console.error("[getAdminToken] Failed to parse admin data:", e);
  }
  return null;
}
...
function getAuthHeaders(contentType = false): Record<string, string> {
  ...
  if (token) {
    ...
    console.warn("[getAuthHeaders] No admin token found in localStorage");
  }
  return headers;
}

// ─── Helper: Handle API response with auth error checking ────────────────────────
async function handleApiResponse<T>(response: Response, defaultValue: T): Promise<T> {
  ...
  if (response.status === 403) {
    console.error("[API] 403 Forbidden - admin access required");
...
function AdminDashboard({ admin }: { admin: { name: string; email: string } }) {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const token = getAdminToken();
      if (!token) {
        console.error("[fetchAllData] No admin token found, redirecting to login");
        showToast("Session expired. Please login again.", "error");
        router.push("/admin/login");
        return;
      }

      console.log("[fetchAllData] Fetching with token:", token.substring(0, 20) + "...");
      ...
      if (bookingsRes.status === 401 || statsRes.status === 401 || roomsRes.status === 401) {
        console.error("[fetchAllData] 401 Unauthorized - clearing session");
      ...
      console.log("[fetchAllData] Responses:", {
        bookings: bookingsData.success,
        stats: statsData.success,
        rooms: roomsData.success,
      });
