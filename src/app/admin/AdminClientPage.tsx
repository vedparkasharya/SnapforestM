...
import {
  ...
  Layers, Maximize2, Compass, Sun, Moon, Droplets, Wind, ToggleLeft, ToggleRight,
} from "lucide-react";
...
const DEMO_IMAGES = [
  ...
  "/rooms/podcast-hub.jpg",
  "/rooms/youtube-studio.jpg",
  "/rooms/music-room.jpg",
  "/rooms/photo-lab.jpg",
  "/rooms/gaming-zone.jpg",
  "/rooms/interview-room.jpg",
  "/rooms/reel-studio.jpg",
];

function getDemoImage(index: number): string {
  return DEMO_IMAGES[index % DEMO_IMAGES.length] || DEMO_IMAGES[0];
}

/**
 * Convert uploaded base64 images to local blob URLs for demo mode
 * When Cloudinary is not configured, this allows showing images without external upload
 */
async function processImagesForDemo(base64Images: string[]): Promise<string[]> {
  // In demo mode, we keep the base64 data URIs directly - browsers can render them
  // This allows the teacher to see the uploaded images without Cloudinary
  return base64Images.filter((img) => img && img.startsWith("data:image"));
}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {demoMode && (
            <Badge variant="warning" className="text-xs">Demo Mode - Images stored locally</Badge>
          )}
      ...
      <p className="text-xs text-muted-foreground">
        {demoMode
          ? "Demo mode active - Images will be shown as preview without external upload. Click or drag & drop JPG files. Max 5MB each."
          : "Upload photos from all angles. Click or drag & drop JPG files. Max 5MB each."}
      </p>
...
// ─── Demo Room Data for Teacher Presentation ──────────
function generateDemoRoom(formData: FormData, imageSlots: ImageSlot[]): Room {
  ...
  // Use base64 images directly in demo mode, or fallback to demo images
  const finalImages = base64Images.length > 0
    ? base64Images
    : [getDemoImage(Math.floor(Math.random() * DEMO_IMAGES.length))];

  return {
    _id: `demo_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
    name: (formData.get("name") as string) || "Demo Room",
    slug: (formData.get("slug") as string) || `demo-room-${Date.now()}`,
    description: (formData.get("description") as string) || "Demo room for presentation",
    category: (formData.get("category") as string) || "podcast",
    city: (formData.get("city") as string) || "Patna",
    address: (formData.get("address") as string) || "Demo Address, Patna",
    images: finalImages,
    equipment: ((formData.get("equipment") as string) || "Microphone, Camera")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean),
    pricePerHour: Number(formData.get("pricePerHour")) || 500,
    pricePerDay: Number(formData.get("pricePerDay")) || 3000,
    featured: formData.get("featured") === "on",
    rating: 4.5,
    reviews: 0,
    capacity: Number(formData.get("capacity")) || 5,
    isAvailable: true,
    mapLink: (formData.get("mapLink") as string) || undefined,
    createdAt: now,
  };
}
...
export default function AdminClientPage() {
  ...
  const [demoMode, setDemoMode] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("snapforest_demo_mode") === "true";
    }
    return false;
  });
  ...
  // Persist demo mode
  useEffect(() => {
    localStorage.setItem("snapforest_demo_mode", demoMode.toString());
  }, [demoMode]);
  ...
  return (
    <>
      ...
      <ScrollProgress />
      <AdminDashboard
        admin={{ name: user?.name || "Admin", email: user?.email || "" }}
        demoMode={demoMode}
...
function AdminDashboard({
  ...
  // Demo rooms state (stored locally when in demo mode)
  const [demoRooms, setDemoRooms] = useState<Room[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("snapforest_demo_rooms");
      if (saved) {
  ...
  // Persist demo rooms
  useEffect(() => {
    if (demoMode && demoRooms.length > 0) {
      localStorage.setItem("snapforest_demo_rooms", JSON.stringify(demoRooms));
    }
  }, [demoRooms, demoMode]);
  ...
  const fetchAllData = async () => {
    setLoading(true);
    try {
      // In demo mode, skip API calls and use local data
      if (demoMode) {
        ...
        setStats(demoStats);
        setRooms(demoRooms.length > 0 ? demoRooms : []);
        setBookings([]);
        setLoading(false);
        return;
      }
      ...
      const [bookingsRes, statsRes, roomsRes] = await Promise.all([
        ...
        fetch("/api/admin/rooms", { headers: getAuthHeaders() }),
      ]);

      // Handle 401 unauthorized - redirect to login
      if (bookingsRes.status === 401 || statsRes.status === 401 || roomsRes.status === 401) {
      ...
      if (!demoMode) {
        showToast("Failed to load dashboard data. Switch to Demo Mode to preview features.", "error");
      }
  ...
  const exportToCSV = () => {
    const headers = ["Booking ID", "Guest Name", "Guest Email", "Guest Phone", "Room", "Date", "Time", "Amount", "Status", "Payment", "Purpose", "Notes"];
  ...
  const handleCreateRoom = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!formData.get("name") || !formData.get("slug") || !formData.get("category")) {
        showToast("Please fill in all required fields (Name, Slug, Category)", "error");
        setSubmitting(false);
        return;
      }

      // ─── Demo Mode: Store room locally ───────────────────
      if (demoMode) {
        ...
        showToast("Demo room created successfully! (Stored locally)", "success");
        setSubmitting(false);
        return;
      }
      ...
      // Safely parse equipment - handle null/empty values
      ...
      console.log("[Create Room] Sending data:", JSON.stringify(data, null, 2));
      const res = await fetch("/api/admin/rooms", { method: "POST", headers: getAuthHeaders(true), body: JSON.stringify(data) });
      ...
      console.error(error);
      showToast("Network error. Try enabling Demo Mode.", "error");
    }
    setSubmitting(false);
  };
  ...
  const handleUpdateRoom = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (demoMode) {
        const updatedRoom: Room = {
          ...editingRoom,
          name: (formData.get("name") as string) || editingRoom.name,
          slug: (formData.get("slug") as string) || editingRoom.slug,
          description: (formData.get("description") as string) || editingRoom.description,
          category: (formData.get("category") as string) || editingRoom.category,
          city: (formData.get("city") as string) || editingRoom.city,
          address: (formData.get("address") as string) || editingRoom.address,
          pricePerHour: Number(formData.get("pricePerHour")) || editingRoom.pricePerHour,
          pricePerDay: Number(formData.get("pricePerDay")) || editingRoom.pricePerDay,
          capacity: Number(formData.get("capacity")) || editingRoom.capacity,
          featured: formData.get("featured") === "on",
          isAvailable: formData.get("isAvailable") === "on",
          mapLink: (formData.get("mapLink") as string) || editingRoom.mapLink,
          equipment: ((formData.get("equipment") as string) || "")
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean),
        };
        setDemoRooms((prev) => prev.map((r) => (r._id === editingRoom._id ? updatedRoom : r)));
        setRooms((prev) => prev.map((r) => (r._id === editingRoom._id ? updatedRoom : r)));
        setEditingRoom(null);
        showToast("Demo room updated successfully!", "success");
        setSubmitting(false);
        return;
      }
      ...
      console.log("[Update Room] Sending data:", JSON.stringify(data, null, 2));
      const res = await fetch(`/api/admin/rooms/${editingRoom._id}`, { method: "PUT", headers: getAuthHeaders(true), body: JSON.stringify(data) });
      const result = await res.json();
    ...
    } catch (error) { console.error(error); showToast("Network error. Try Demo Mode.", "error"); }
    setSubmitting(false);
  };
  ...
  const handleDeleteRoom = async (roomId: string) => {
    ...
    if (demoMode) {
      ...
      setDemoRooms((prev) => prev.filter((r) => r._id !== roomId));
      setRooms((prev) => prev.filter((r) => r._id !== roomId));
      setDeleteConfirm(null);
      showToast("Demo room deleted!", "success");
    ...
    setDeleteLoading(true);
    try {
      const res = await fetch(`/api/admin/rooms/${roomId}`, { method: "DELETE", headers: getAuthHeaders() });
      const data = await res.json();
  ...
  const handleRefund = async (bookingId: string) => {
    ...
    try {
      const res = await fetch("/api/admin/bookings/refund", { method: "POST", headers: getAuthHeaders(true), body: JSON.stringify({ bookingId }) });
      const data = await res.json();
  ...
  // Include demo rooms in the rooms list when in demo mode
  const displayRooms = demoMode
    ? [...demoRooms, ...rooms.filter((r) => !r._id.startsWith("demo_"))]
    : rooms;

  const filteredDisplayRooms = displayRooms.filter((r) =>
    r.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    r.category?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    r.city?.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-2 flex-wrap w-full lg:w-auto">
            ...
            <button
              ...
              title={demoMode ? "Demo Mode is ON - Rooms stored locally" : "Enable Demo Mode to test without Cloudinary"}
            >
              {demoMode ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
              <span className="hidden sm:inline">Demo {demoMode ? "ON" : "OFF"}</span>
            </button>
        ...
        <AnimatePresence>
          {demoMode && (
            ...
            >
              <div className="glass-card p-4 mb-6 flex items-center justify-between bg-gradient-to-r from-yellow-500/10 via-amber-500/10 to-orange-500/10 border border-yellow-500/20">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-400 to-amber-500 flex items-center justify-center animate-pulse">
                    <Sparkles className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <p className="font-semibold text-sm text-yellow-400">Demo Mode Active</p>
                    <p className="text-xs text-muted-foreground">
                      Rooms are stored locally in your browser. No Cloudinary or database required.
                      Perfect for teacher presentations!
                    </p>
                  </div>
                </div>
                <Button variant="outline" size="sm" onClick={() => onDemoModeChange(false)} className="border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/10">
                  Disable
                </Button>
              </div>
            </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 glass-card mb-6 gap-3 border border-white/10 hover:border-neon-cyan/20 transition-all">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {demoMode && (
              <Badge variant="warning" className="text-xs bg-gradient-to-r from-yellow-500/20 to-amber-500/20">Demo Mode</Badge>
            )}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white/5 mb-6 flex-wrap h-auto gap-1 border border-white/10 p-1">
            ...
            <TabsTrigger value="rooms" className="gap-1.5 data-[state=active]:bg-gradient-to-r data-[state=active]:from-neon-cyan/20 data-[state=active]:to-neon-purple/20 data-[state=active]:text-neon-cyan"><Eye className="w-3.5 h-3.5" />Rooms ({filteredDisplayRooms.length})</TabsTrigger>
              ...
              <div className="glass-card overflow-hidden border border-white/10 hover:border-neon-cyan/20 transition-all">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="border-b border-white/5 text-muted-foreground"><th className="text-left p-4">Guest</th><th className="text-left p-4">Room</th><th className="text-left p-4">Date</th><th className="text-left p-4">Amount</th><th className="text-left p-4">Status</th></tr></thead>
                    <tbody>
                      {bookings.slice(0, 5).map((booking) => (
                        <tr key={booking._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                          <td className="p-4">
                            <div className="font-medium">{booking.guestName || booking.user?.name || "N/A"}</div>
                            <div className="text-xs text-muted-foreground">{booking.guestEmail || booking.user?.email || "N/A"}</div>
                          </td>
                      ...
                      {bookings.length === 0 && <tr><td colSpan={5} className="text-center py-12 text-muted-foreground">{demoMode ? "Demo mode - No bookings yet" : "No bookings found"}</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      ...
                      {displayRooms.length === 0 && <tr><td colSpan={4} className="text-center py-12 text-muted-foreground">{demoMode ? "Click 'Add Room' to create demo rooms!" : "No rooms found"}</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          <TabsContent value="bookings">
            <div className="glass-card overflow-hidden border border-white/10 hover:border-neon-cyan/20 transition-all">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-white/5 text-muted-foreground"><th className="text-left p-4">Guest</th><th className="text-left p-4">Contact</th><th className="text-left p-4">Room</th><th className="text-left p-4">Date & Time</th><th className="text-left p-4">Amount</th><th className="text-left p-4">Status</th><th className="text-left p-4">Purpose</th><th className="text-left p-4">Actions</th></tr></thead>
                  <tbody>
                    {paginatedBookings.map((booking) => (
                      <tr key={booking._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        <td className="p-4">
                          <div className="font-medium">{booking.guestName || booking.user?.name || "N/A"}</div>
                          {booking.bookingId && <div className="text-[10px] text-muted-foreground">{booking.bookingId}</div>}
                        </td>
                        <td className="p-4">
                          <div className="text-xs text-muted-foreground">{booking.guestEmail || booking.user?.email || "N/A"}</div>
                          <div className="text-xs text-muted-foreground">{booking.guestPhone || "N/A"}</div>
                        </td>
                        <td className="p-4">{booking.room?.name}</td>
                        ...
                        <td className="p-4">
                          <div className="flex flex-col gap-1">
                            <Badge variant={booking.status === "confirmed" ? "success" : booking.status === "cancelled" ? "destructive" : "warning"}>{booking.status}</Badge>
                            <Badge variant={booking.paymentStatus === "paid" ? "success" : "secondary"}>{booking.paymentStatus}</Badge>
                          </div>
                        </td>
                        <td className="p-4 text-xs text-muted-foreground">{booking.purpose || "-"}</td>
                    ...
                    {paginatedBookings.length === 0 && <tr><td colSpan={8} className="text-center py-12 text-muted-foreground">{searchQuery ? "No bookings match your search" : demoMode ? "Demo mode - Create demo rooms to see bookings" : "No bookings found"}</td></tr>}
                  </tbody>
                </table>
              </div>
              {totalPages > 1 && (
          ...
          <TabsContent value="rooms">
            <div className="glass-card overflow-hidden border border-white/10 hover:border-neon-purple/20 transition-all">
              <div className="p-4 sm:p-6 border-b border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h2 className="text-lg sm:text-xl font-semibold flex items-center gap-2"><Eye className="w-5 h-5 text-neon-purple" />All Rooms ({filteredDisplayRooms.length})</h2>
                {demoMode && (
                  <Badge variant="warning" className="text-xs w-fit">Demo Mode - {demoRooms.length} demo room{demoRooms.length !== 1 ? "s" : ""} stored locally</Badge>
                )}
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                      ...
                      <tr key={room._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="w-3 h-3" />{room.address}</div>
                            </div>
                          </div>
                        </td>
                        <td className="p-4"><Badge variant="neon" className="text-xs">{categoryLabels[room.category] || room.category}</Badge></td>
                        ...
                        <td className="p-4">
                          <div className="flex flex-col gap-1">
                            ...
                            {room._id.startsWith("demo_") && <Badge variant="warning" className="text-xs w-fit">Demo</Badge>}
                          </div>
                        </td>
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                    ...
                    {filteredDisplayRooms.length === 0 && <tr><td colSpan={8} className="text-center py-12 text-muted-foreground">{searchQuery ? "No rooms match your search" : demoMode ? "Click 'Add Room' to create your first demo room!" : "No rooms found"}</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto glass-card border-neon-cyan/20">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-xl">
                ...
                Add New Room
                {demoMode && <Badge variant="warning" className="ml-2 text-xs">Demo</Badge>}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleCreateRoom} className="space-y-5 mt-4">
              <RoomFormFields />
              <div className="border-t border-white/10 pt-4">
                <RoomImageUploader slots={imageSlots} onSlotsChange={setImageSlots} demoMode={demoMode} />
              </div>
              <Button type="submit" variant="neon" className="w-full hover:shadow-[0_0_30px_rgba(0,245,212,0.3)] transition-all" disabled={submitting}>
                ...
                {demoMode ? "Create Demo Room" : "Create Room"}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto glass-card border-neon-purple/20">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-xl">
                ...
                Edit Room
                {editingRoom?._id.startsWith("demo_") && <Badge variant="warning" className="ml-2 text-xs">Demo</Badge>}
              </DialogTitle>
            </DialogHeader>
            {editingRoom && (
              <form onSubmit={handleUpdateRoom} className="space-y-5 mt-4">
                ...
                <div className="border-t border-white/10 pt-4">
                  <RoomImageUploader slots={editImageSlots} onSlotsChange={setEditImageSlots} demoMode={demoMode} />
                </div>
                ...
                <Button type="submit" variant="neon" className="w-full hover:shadow-[0_0_30px_rgba(155,93,229,0.3)] transition-all" disabled={submitting}>
                  ...
                  {demoMode ? "Update Demo Room" : "Update Room"}
                </Button>
              </form>
            )}
          </DialogContent>
